...
export function CommunityEngagementBar({
  ...
  return (
    <div className={className}>
      <div
        className={cn("flex flex-wrap items-center gap-1.5", isPrayer && "gap-2")}
        role="group"
        aria-label={isPrayer ? "Prayer responses" : "Post reactions"}
      >
        {allowReactions
          ...
          <button
            ...
            className={cn(
              ...
              commentsOpen
                ...
                : isPrayer
                  ? "bg-white/85 text-brand-ink/72 border-black/[0.05] hover:border-brand-primary/15"
                  : "bg-white/80 text-brand-ink/70 border-black/[0.06] hover:border-brand-primary/20",
            )}
          >
            {isPrayer ? (
              <HandHeart className={cn(compact ? "h-3.5 w-3.5" : "h-4 w-4")} aria-hidden />
            ) : (
